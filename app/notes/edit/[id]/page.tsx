...
import { SlashCommandMenu } from "../../new/slash-command-menu";
...
export default function EditNotePage() {
  ...
  useEffect(() => {
    if (noteId) {
      ...
      const note = savedNotes.find((n: Note) => n.id === parseInt(noteId));
      if (note) {
        setTitle(note.title);
  ...
  const handleSave = useCallback(() => {
    ...
    const duplicateNote = savedNotes.find((note: Note) => 
    ...
    const updatedNotes = savedNotes.map((note: Note) =>
      note.id === parseInt(noteId)
